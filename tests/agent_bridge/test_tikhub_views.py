...
def test_every_tikhub_state_is_correlated_revisioned_and_separate_from_mcp() -> None:
    ...
    fixtures = {
        ...
        "tikhub.health": {"state":"healthy","route_mode":"direct_https","checked_at_ms":1,"latency_ms":2,"provider_request_id":None,"error_code":None},
        ...
        "tikhub.route": {"route_policy_ref":"route:tikhub-direct-https-v1","mode":"direct_https","state":"ready","remote_dns":"not_applicable","leak_check":"passed","exit_identity":"not_applicable","attested_at_ms":1,"error_code":None},
...
def test_stale_incompatible_and_forbidden_fields_preserve_last_known_valid_state() -> None:
    console = AgentConsoleEngine()
    valid = event("tikhub.health", 2, {"state":"degraded","route_mode":"direct_https","checked_at_ms":1,"latency_ms":2,"provider_request_id":None,"error_code":"UPSTREAM_UNAVAILABLE"})
...
def test_startup_republish_rehydrates_a_fresh_console_within_two_seconds() -> None:
    ...
    republished = AgentEvent(
        "tikhub.health",
        {"revision": 8, "state": "degraded", "route_mode": "direct_https",
